...
from src.momentumexe.motion_execution import desired_state
...
def create_file(time_vector, optimized_sequence):
    ...
    if using_quadruped:
        ...
        for i in range(num_points):
            des_positions[i, :] = np.hstack((i, desired_pos(i / sample_frequency)))
            des_velocities[i, :] = np.hstack((i, desired_vel(i / sample_frequency)))
