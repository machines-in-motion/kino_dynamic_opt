...
using namespace momentumopt;
  ...
  class Solo8MotionTests : public ::testing::Test
  {
  protected:
    virtual void SetUp() {}
  ...
  void testProblem(const std::string cfg_file, const ExitCode expected_code, const bool& tinfo)
  {
    ...
    if (tinfo)
    {
      std::cout << "  Timesteps:" << std::fixed << std::setw(4) << std::setprecision(0) << planner_setting.get(PlannerIntParam_NumTimesteps)
  ...
  TEST_F(Solo8MotionTests, test_) {
    testProblem(CFG_SRC_PATH+std::string("quadruped_demos/cfg_quadruped_jump.yaml"), ExitCode::Optimal, display_time_info);
  }
